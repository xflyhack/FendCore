...
class ConfigTest extends TestCase
{
    public function testSetConfig()
    {
        ...
        \Fend\Config::setConfigPath(SYS_ROOTDIR . 'example/EmptyConfig');
        ...
        \Fend\Config::clean();
        \Fend\Config::loadConfig('Redis', SYS_ROOTDIR . 'example/Config/Redis.php');
        \Fend\Config::loadConfig('Redis', SYS_ROOTDIR . 'example/Config/Redis.php');
        $host = \Fend\Config::get('Redis.default.host');
        ...
        $test = 0;
        try {
            \Fend\Config::loadConfig('Redis',SYS_ROOTDIR . 'example/Config/Redis');
        } catch (\Exception $e) {
            $test = 1;
        }
        self::assertEquals(1, $test);
        $test = 0;
        try {
            \Fend\Config::loadConfig('Redis',SYS_ROOTDIR . 'example/Config/Test.php');
        } catch (\Exception $e) {
            $test = 1;
        }
        self::assertEquals(1, $test);
